...
export function ServicesSection() {
  return (
    <section id='dich-vu' className='py-[4rem] lg:py-[6rem]'>
      <Container>
        <div className='rounded-[1.25rem] bg-brand-blue px-[1.25rem] py-[1.5rem] text-white sm:px-[1.75rem] sm:py-[2rem]'>
          ...
          <p className='mt-[0.625rem] max-w-[38rem] text-[0.9375rem] leading-[1.7] text-white/90'>
            Danh sách dịch vụ hiển thị dưới đây là dữ liệu giả lập, đủ cấu trúc
            để bạn thay thế bằng dữ liệu thật ở bước tích hợp API sau này.
          </p>
        </div>
        ...
        <div className='mt-[2rem] grid gap-[1rem] lg:grid-cols-2'>
          {services.map((service) => (
            ...
            >
              ...
              <h3 className='mt-[0.75rem] text-[1.125rem] font-semibold text-text-dark-blue'>{service.title}</h3>
              <p className='mt-[0.5rem] text-[0.9375rem] leading-[1.7] text-text-dark-blue/80'>{service.summary}</p>
            </article>
          ))}
        </div>
      </Container>
